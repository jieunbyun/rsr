"""
Run RSR on IEEE 118-bus DC-OPF blackout model (branches + buses).
...
Usage:
    python run_rsr_ieee118.py
    python run_rsr_ieee118.py --unk-prob-thres 1e-4
    python run_rsr_ieee118.py --devices cuda:0,cuda:1
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="RSR on IEEE 118-bus DC-OPF")
...
def main():
    ...
    # ---------------------------------------------------------------
    # 4. Run RSR reference extraction
    # ---------------------------------------------------------------
    output_dir = HERE / "rsr_res"
    sys_upper_st = 1
    print(f"\n  Output:      {output_dir}")
    print(f"  Samples:     1,000,000 per round (batch 10,000)")
    ...
    if metrics_path.exists():
        ...
        print(f"  Upper refs:  {last.get('n_refs_upper', '?')}")
        print(f"  Lower refs:  {last.get('n_refs_lower', '?')}")
        print(f"  Unk prob:    {last.get('p_unknown', '?')}")
        print(f"  P(upper):    {last.get('p_upper', '?')}")
        print(f"  P(lower):    {last.get('p_lower', '?')}")
        p_fail = last.get('p_lower', 0)
        print(f"\n  Reference (Chan et al. Table 2): p_f ~ 1.0e-4")
        print(f"  RSR estimate:                    p_f ~ {p_fail:.2e}")
