"""
draw_figure.py — selectively regenerate figures for whats_rsr.
...
---------------
    ...
    brc_vs_rsr         BRC vs. RSR comparison
    rsr_conn           RSR 1-OD-conn vs. global-conn
    global_conn_gpu     global-conn (CPU) vs. global_conn_gpu comparison
    no_min_ref         RSR 1-OD-conn vs. no-min-reference state (Graph 1)
    ...
    results_summary_unc results summary allowing RSR unclassified samples

Examples
--------
    python draw_figure.py no_min_ref
    python draw_figure.py rsr_conn no_min_ref
    python draw_figure.py all
"""
import json
import sys
...
def draw_brc_monitor_data(
    ...
    n_refs, unk_prob, n_brs, mem, c_time = {}, {}, {}, {}, {}

    for path, label in zip(data_paths, labels):
        with open(path, "r") as f:
            data_ = json.load(f)

        n_refs[label] = data_["no_ra"]
...
def load_data_brc_rsr(
    ...
    if not all(s.endswith("BRC") or s.endswith("RSR") for s in labels):
        raise ValueError("All labels must end with 'BRC' or 'RSR'.")

    sec_to_hr = 1.0 / 3600.0

    n_refs, unk_prob, mem, c_time = {}, {}, {}, {}
    p0_up, p0_low = {}, {}
    ...
    for path, label in zip(data_paths, labels):
        if label.endswith("BRC"):
        ...
        elif label.endswith("RSR"):
            data_ = pd.read_json(path, lines=True)

            n_refs[label] = data_["round"].to_list()
            ...
            p1_ = data_["p_upper"].to_list()
            p0_up[label] = [1.0-p for p in p1_]
            p0_low[label] = data_["p_lower"].to_list()

    return n_refs, unk_prob, mem, c_time, p0_up, p0_low

def draw_brc_vs_rsr_data(
    data_paths: list,
    labels: list,
) -> None:
    """
    ...
    n_refs, unk_prob, mem, c_time, p0_up, p0_low = load_data_brc_rsr(data_paths, labels)
    ...
    _plot_all(
        ...
        marker = [None]*4,
        linestyle=['-', '--', '-', '--'],
        save_fname=HERE / "figs/brc_rsr_refs_vs_log_unc_prob.png",
        ylog=True,
        #xlog=True,
        show_legend=False,
    )
    ...
    _plot_all(
        ...
        marker = [None]*4,
        linestyle=['-', '--', '-', '--'],
        save_fname=HERE / "figs/brc_rsr_refs_vs_mem.png",
        ylog=True,
    )
    ...
    _plot_all(
        ...
        marker = [None]*4,
        linestyle=['-', '--', '-', '--'],
        save_fname=HERE / "figs/brc_rsr_refs_vs_c_time.png",
        ylog=True,
        ylim=[5e-4, 1e3],
    )
    ...
    plot_bounds_dual_y(
        ...
        save_fname=HERE / "figs/brc_rsr_refs_vs_bounds_Phi1_Phi2_dualy.png",
        show_legend=True,
        ylog_L=True,
        ylog_R=True,
        #xlog =True,
...
def draw_rsr_data(
    ...
    _plot_all(
        ...
        markevery=[1] * n,
        marker=[None] * n,
        save_fname=HERE / f"figs/rsr_refs_vs_log_unc_prob{save_suffix}.png",
        ylog=True,
        xlog=True,
        show_legend=False,
        xlim=[1, 1e4+100]
    ...
    _plot_all(
        ...
        markevery=[1] * n,
        marker=[None] * n,
        save_fname=HERE / f"figs/rsr_refs_vs_mem{save_suffix}.png",
        show_legend=True,
        ylog=True,
        xlog=True,
        xlim=[1, 1e4+100],
    ...
    _plot_all(
        ...
        markevery=[1] * n,
        marker=[None] * n,
        save_fname=HERE / f"figs/rsr_refs_vs_c_time{save_suffix}.png",
        ylog=True,
        ylim=[5e-4, 1e3],
        show_legend=False,
        xlog=True,
...
def draw_global_conn_gpu_comparison(
    ...
    cpu_paths : list of Path
        metrics.jsonl files for rsr_global_conn (one per graph).
        x-axis = round number.
    gpu_paths : list of Path
        metrics.json files for rsr_global_conn_gpu (one per graph).
        x-axis = n_refs_upper + n_refs_lower (total reference states found
    ...
    # Same colour as rsr_conn for global-conn; distinct colour for GPU
    cpu_color = "#E69F00"
    ...
    n_refs, total_refs, unk_prob, mem, c_time = {}, {}, {}, {}, {}
    ...
    for path, label in zip(cpu_paths, cpu_labels):
        ...
        n_refs[label] = data_["round"].to_list()
        total_refs[label] = (data_["n_refs_upper"] + data_["n_refs_lower"]).to_list()
        unk_prob[label] = data_["p_unknown"].to_list()
        mem[label] = data_["rss_gb"].to_list()
    ...
    # GPU data: x = n_refs_upper + n_refs_lower (total reference states found so far)
    # Prepend [0, epsilon] so lines start from the left edge on log-log plots.
    _eps = 1e-10
    for path, label in zip(gpu_paths, gpu_labels):
        data_ = _read_metrics(path)
        n_refs[label] = [0] + (data_["n_refs_upper"] + data_["n_refs_lower"]).to_list()
        total_refs[label] = n_refs[label]
    ...
    header = f"{'Label':<30} {'Final # reference states':>14} {'Total time (hr)':>16} {'Final RSS (GB)':>15} {'Final unk. prob':>16}"
    print(header)
    print("-" * len(header))
    for label in all_labels:
        print(
            f"{label:<30} "
            f"{total_refs[label][-1]:>14d} "
...
def get_results_summary(
    ...
    n_refs_summary = [n_refs[k][-1] for k in labels]
    ...
    for l, r, g in zip(labels, refs_paths, graph_paths):
        ...
        if l.endswith("BRC"):
            ...
            refs_mat_upper = torch.load(r / f"refs_up_1.pt", map_location="cpu")
            refs_mat_upper = refs_mat_upper.to(device)
            refs_mat_lower = torch.load(r / f"refs_low_0.pt", map_location="cpu")
            refs_mat_lower = refs_mat_lower.to(device)

            print(f"Evaluating system probability for {l}...")
            st = time.time()
            psys = rsr.get_comp_cond_sys_prob(
                        refs_mat_upper,
                        refs_mat_lower,
                        probs,
                        comps_st_cond = {},
            ...
            print(f"{l} evaluation time: {end-st:.2f} seconds:")
            print(f"P(sys >= 1) = {psys['upper']:.3e}")
            print(f"P(sys <= 0) = {psys['lower']:.3e}\n")

        sys_prob_time_summary.append(end-st)

    summary = pd.DataFrame({
        "Label": labels,
        "Final number of reference states": n_refs_summary,
...
def get_direct_results_summary(
    ...
    for k in labels:
        ...
        n_refs_summary.append(n_refs[k][last_i])
    ...
    summary = pd.DataFrame({
        "Label": labels,
        "Number of reference states at threshold": n_refs_summary,
...
def get_results_summary_rsr_unc(
    data_paths: list,
    labels: list,
    refs_paths: list,
    graph_paths: list,
    ...
    """
    Allow for RSR to return unknown probability > 0
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Selectively regenerate figures for numerical examples with random graphs.",
    ...
    # --- BRC vs. RSR ---
    if "brc_vs_rsr" in run:
        draw_brc_vs_rsr_data(
            data_paths=[
                HERE.parent / "results/rg1/v1/brc/monitor_conn.json",
                HERE.parent / "results/rg2/v1/brc/monitor_conn.json",
                HERE.parent / "results/rg1/v1/rsr_conn/metrics.jsonl",
                HERE.parent / "results/rg2/v1/rsr_conn/metrics.jsonl",
            ],
            labels=["Graph 1; BRC", "Graph 2; BRC", "Graph 1; RSR", "Graph 2; RSR"],
        )

    # --- RSR: 1-OD-conn vs. global-conn ---
    if "rsr_conn" in run:
        draw_rsr_data(
            data_paths=[
                HERE.parent / "results/rg1/v1/rsr_conn/metrics.jsonl",
                HERE.parent / "results/rg2/v1/rsr_conn/metrics.jsonl",
                HERE.parent / "results/rg1/v1/rsr_global_conn/metrics.jsonl",
                HERE.parent / "results/rg2/v1/rsr_global_conn/metrics.jsonl",
            ],
    ...
    if "global_conn_gpu" in run:
        draw_global_conn_gpu_comparison(
            ...
            gpu_paths=[
                HERE.parent / "results/rg1/v1/rsr_global_conn_gpu/metrics.json",
                HERE.parent / "results/rg2/v1/rsr_global_conn_gpu/metrics.json",
            ],
        )

    # --- RSR: 1-OD-conn vs. no-min-reference state (Graph 1) ---
    if "no_min_ref" in run:
        draw_rsr_data(
            data_paths=[
                HERE.parent / "results/rg1/v1/rsr_conn/metrics.jsonl",
                HERE.parent / "results/rg1/v1/rsr_conn_no_min_rule/metrics.jsonl",
            ],
            labels=["Graph 1; 1 OD", "Graph 1; No min reference state"],
            colors=["black", "#56B4E9"],
            save_suffix="_no_min_ref",
        )
    ...
    if "direct_summary" in run:
        get_direct_results_summary(
            ...
            labels=[
                "Graph 1; 1 OD; BRC", "Graph 1; 1 OD; RSR", "Graph 1; Global; RSR",
                "Graph 2; 1 OD; BRC", "Graph 2; 1 OD; RSR", "Graph 2; Global; RSR",
            ],
            save_path=HERE / "figs/direct_results_summary.csv",
        )

    # --- Results summary allowing RSR unclassified samples ---
    if "results_summary_unc" in run:
        get_results_summary_rsr_unc(
            data_paths=[HERE.parent / "results/rg2/v1/rsr_global_conn/metrics.jsonl"],
            labels=["Graph 2; Global; RSR"],
            refs_paths=[HERE.parent / "results/rg2/v1/rsr_global_conn"],
